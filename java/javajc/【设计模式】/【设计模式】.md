...
```java
1.XML的语法
    文档声明: 固定格式<?xml version="1.0" encoding="UTF-8"?>
    元素/标签: 
...
2.多例设计模式[必会]
3.工厂设计模式[必会]
4.动态代理[重点掌握API] 3次 第一次掌握API 第二次在框架中使用  第三次在项目中使用 
5.Lombok idea一款插件 
    我们在写一个标准类是都需要get/set,无参+满参,toString,hashCOde,equals
    我们使用idea的快捷alt+insert 半自动生成
    Lombok 会全自动生成以上方法 
```
...
能够使用Proxy的方法生成代理对象
    Proxy.newProxyInstance(
        类加载器,
        被代理对象所实现的接口的字节码数组,
        InvocationHandler接口实现类对象
    );
	public interface InvocationHandler{
        //拦截我们对象生成的代理对象调用的一切方法
     	public Object invoke(代理对象,被拦截住的方法对象,被拦截住的方法参数);   
    }
