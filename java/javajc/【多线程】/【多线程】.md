...
2.二分查找[理解]
    public static int searchKey(int[] arr,int key){
    	...
    	while(min <= max){
            int midlle = (min+max)/2;
            if(arr[middle] == key){
                retrun middle;
            }else if(arr[middle] > key){
                //中间值大于key,说明接下来,我们要找左半段
                max = middle - 1;
            }else{
                //中间值小于key,说明接下来,我们要找右半段]
                min = middle + 1;
            }
        }
    	return -1;
...
3.异常[重点]    
   a.异常的继承体系
    	Throwable 根类  
    		子类:Exception
                编译器异常: Exception以及其子类(RuntimeException除外)
				运行时异常: RuntimeException以及其子类
            子类:Error 
  b.异常的3个通用方法
        public void printStackTrace();打印当前异常对象的详细信息
        #public String getMessage();获取异常的部分信息
        #public String toString();重写Object的toString     
  c.和异常相关的五个关键字[重点]
        throw: 动词,抛出异常
               throw new XxxException("该异常的一些信息");
		throws: 形容词,表示可能有异常抛出  
